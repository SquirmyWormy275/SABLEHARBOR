...
def requests():
    ...
    add(
        "us_states",
        TIGER + "State_County/MapServer/0/query",
        {"where": "STATE NOT IN ('02','15','60','66','69','72','78')", "maxAllowableOffset": 0.02},
        "U.S. Census Bureau",
        "Generalized continental US context; not for county or parcel containment.",
        "0.02 degree generalization",
    )
    add(
        "wyoming_counties",
        TIGER + "State_County/MapServer/1/query",
        {"where": "STATE='56'", "maxAllowableOffset": 0.001},
        "U.S. Census Bureau",
        "Wyoming county context. Unsimplified point-containment response is separately archived.",
        "0.001 degree generalization",
    )
    add(
        "context_places",
        TIGER + "tigerWMS_Current/MapServer/28/query",
        {
            "where": "(STATE='56' AND NAME LIKE 'Wamsutter%') OR (STATE='06' AND NAME LIKE 'Sacramento%') OR (STATE='42' AND NAME LIKE 'Pittsburgh%') OR (STATE='54' AND (NAME LIKE 'Belle %' OR NAME LIKE 'Charleston%'))",
            "maxAllowableOffset": 0.0001,
        },
        "U.S. Census Bureau",
        "Incorporated place boundaries. These are reference municipalities, never company footprints.",
        "0.0001 degree generalization",
    )
    for area, bbox in EXTENTS.items():
        ...
        for kind, layer in [
            ...
            add(
                ...
                TNM + str(layer) + "/query",
                spatial,
                "USGS The National Map / U.S. Census Bureau / U.S. Forest Service",
                "Reference transportation geometry; names do not prove current ownership, operating rights, track condition, or interchange authority.",
                "native service geometry rounded to six decimals",
            )
        if area in {"sacramento", "hazelwood"}:
            add(
                ...
                spatial,
                "USGS The National Map / U.S. Census Bureau / U.S. Forest Service",
                "Reference local roads; no fictional property interest.",
                "native service geometry rounded to six decimals",
            )
        add(
            area + "_hydro",
            HYDRO + ("4" if area == "wamsutter" else "6") + "/query",
            spatial,
            "USGS National Hydrography Dataset",
            "Hydrography context; not a floodplain, wetland, or navigability determination.",
            "native service geometry rounded to six decimals",
        )
    add(
        "wamsutter_fra_rail",
        ...
        {
            "geometry": "-108.15,41.6,-107.80,41.75",
            "geometryType": "esriGeometryEnvelope",
            "inSR": 4326,
            "spatialRel": "esriSpatialRelIntersects",
        },
        "Federal Railroad Administration / Bureau of Transportation Statistics",
        "FRA reference railroad ownership and topology, no fictional interchange rights.",
        "native service geometry rounded to six decimals",
    )
    for name, endpoint, authority, notes in [
        (
            ...
            HYDRO + "12/query",
            "USGS National Hydrography Dataset",
            "Waterbody screening polygons; not wetlands, floodplain or site permission.",
        ),
        (
            "wyoming_local_roads",
            "https://cartowfs.nationalmap.gov/arcgis/rest/services/transportation/MapServer/7/query",
            "USGS The National Map / U.S. Census Bureau",
            "Reference local roads near approved Red Wash anchor; no access rights inferred.",
        ),
    ]:
        add(
            name,
            endpoint,
            {
                "geometry": "-108.38,41.62,-107.82,42.30",
                "geometryType": "esriGeometryEnvelope",
                "inSR": 4326,
                "spatialRel": "esriSpatialRelIntersects",
            ...
            authority,
            notes,
            "Six decimals and 0.00002 degree generalization; screening only",
        )
    return result
...
def fetch(spec):
    try:
        ...
        for start in range(0, len(obj_ids), 500):
            obj, raw, url = read(
                spec["endpoint"],
                {**params, "objectIds": ",".join(map(str, obj_ids[start : start + 500]))},
            )
        ...
        result = {
            **spec,
            "source_id": sid,
            "access_state": "FETCHED",
            "accessed_date": "2026-09-06",
            "feature_count": len(features),
            "response_hashes": raw_hashes,
            "request_urls": urls,
            "file": str(file.relative_to(BASE)),
            "file_sha256": hashlib.sha256(data).hexdigest(),
            "license": "U.S. federal government geographic data; public domain",
            "attribution": spec["authority"],
            "license_url": "https://www.usgs.gov/information-policies-and-instructions/copyrights-and-credits"
            if "USGS" in spec["authority"]
            else "https://www.census.gov/about/policies/copyright.html",
        }
        ...
        if spec["authority"].startswith("Federal Railroad"):
            result["license_url"] = (
                "https://railroads.dot.gov/rail-network-development/maps-and-data/maps-geographic-information-system/maps-geographic"
            )
...
def main():
    ...
    BASE.joinpath("sources/reference_manifest.json").write_text(
        json.dumps(results, indent=2) + "\n"
    )
    params = {
        "f": "geojson",
        "geometry": "-106.9213,42.3127",
    ...
    BASE.joinpath("sources/red_wash_county_check.json").write_text(
        json.dumps(
            {
                "request_url": url,
                "response": obj,
                ...
                "accessed_date": "2026-09-06",
                "method": "Server-side point intersection with unsimplified county geometry",
            },
            indent=2,
        )
        + "\n"
