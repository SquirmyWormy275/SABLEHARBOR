...
def state_filter(records, as_of, owner=None, include_possible=False):
    ...
    return [
        r
        for r in records
        if temporal_membership(r, as_of) in accepted
        and (owner is None or r.get("owner_entity") == owner)
    ]
...
def network_errors(nodes, segments, routes, tolerance_m=2.0):
    ...
    for segment in segments:
        ...
        seen[key] = name
        if (
            p.get("geometry_miles") is not None
            and abs(p["geometry_miles"] - GEOD.geometry_length(g) / 1609.344) > 0.001
        ):
    ...
    for route in routes:
        ...
        for sid in ids:
            if sid not in edge_map:
                ...
                continue
            p = edge_map[sid]["properties"]
            a = p.get("from_node_id")
            b = p.get("to_node_id")
            graph[a].add(b)
            graph[b].add(a)
        if graph:
            ...
            if len(reached) != len(graph):
                errors.append(f"{route['route_id']}: disconnected route")
