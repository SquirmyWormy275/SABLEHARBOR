...
FIELDS = {
    ...
    "fictionality": "TEXT NOT NULL",
    "real_world_relation": "TEXT NOT NULL",
    ...
    "decision_id": "TEXT REFERENCES decision_registry(decision_id)",
    "valid_from": "TEXT",
    "valid_to": "TEXT",
    "recorded_at": "TEXT NOT NULL",
    "superseded_at": "TEXT",
    "source_effective_date": "TEXT",
    "world_state_date": "TEXT",
    "owner_entity": "TEXT",
    ...
    "route_miles": "REAL",
    "geometry_miles": "REAL",
    "geometry_area_acres": "REAL",
    "maximum_grade_pct": "REAL",
...
def reference_source(r):
    return dict(
        ...
        title=r["name"],
        publisher_or_author=r["authority"],
        source_type="REAL_REFERENCE_GIS",
        publication_date=None,
        accessed_date=r["accessed_date"],
        url_or_repo_path=r["endpoint"],
        repository_commit=None,
        file_sha256=r["file_sha256"],
        license=r["license"],
        citation=r["attribution"],
        coverage=r["notes"],
        source_quality="FEDERAL_REFERENCE_SNAPSHOT",
        notes="Query details, response hashes, license URL and extraction precision: sources/reference_manifest.json. Dataset epoch is not a corporate effective date.",
    )
...
def layers(catalog, refs):
    ...
    for stem, name in catalog["geometry_layers"].items():
        result.append(
            (name, json.loads((BASE / "geojson" / (stem + ".geojson")).read_text())["features"])
        )
    for ref in refs:
        if ref["access_state"] != "FETCHED":
            continue
        if digest(BASE / ref["file"]) != ref["file_sha256"]:
            raise ValueError(f"Reference hash mismatch: {ref['name']}")
        ...
        for feature in features:
            ...
            p = {
                **props,
                ...
                "geometry_status": "REFERENCE_SNAPSHOT",
                "location_method": "DIGITIZED_FROM_SOURCE",
                "precision_class": "REFERENCE_DATASET",
            ...
            if not g.is_valid:
                ...
                p["construction_method"] = (
                    "Shapely make_valid of an invalid federal extraction; original source geometry and hash retained unchanged."
                )
                p["geometry_repair_area_delta_sq_degrees"] = fixed.area - g.area
                p["notes"] += (
                    " Source geometry required a documented validity repair; do not treat as cadastral precision."
                )
    ...
    with sqlite3.connect(temporary) as db:
        ...
        for row in [
            ...
            (
                ...
                "EPSG",
                4326,
                CRS.from_epsg(4326).to_wkt(version="WKT1_GDAL"),
        ...
        for row in sorted(
            catalog["sources"]
            + [reference_source(r) for r in refs if r["access_state"] == "FETCHED"],
            key=lambda r: r["source_id"],
        ):
            insert(db, "source_registry", row)
        for table, key in [
            ...
            if obj["object_type"] == "EVENT":
                insert(
                    ...
                    dict(
                        ...
                        event_type="CORPORATE_HISTORY",
                        event_date=obj["relevant_date"] if obj["date_precision"] == "DAY" else None,
                        event_end_date=None,
                        date_text=obj["relevant_date"],
                        date_precision=obj["date_precision"],
                        entity_ids=[obj["entity_id"]] if obj["entity_id"] else [],
                        asset_ids=[],
                        canon_status=obj["canon_status"],
                        source_id=obj["source_id"],
                        decision_id=obj["decision_id"],
                        description=obj["exact_source_wording"],
                        notes=obj["notes"],
                    ),
                )
        ...
        for name, features in layers(catalog, refs):
            ...
            db.execute(
                f'CREATE TABLE "{name}" (fid INTEGER PRIMARY KEY AUTOINCREMENT,geom GEOMETRY NOT NULL,{sql},CHECK(valid_to IS NULL OR valid_from IS NULL OR valid_to>valid_from))'
            )
            bounds = []
            for feature in sorted(features, key=lambda f: f["properties"]["feature_id"]):
                g = shape(feature["geometry"])
                if g.is_empty or not g.is_valid:
                    raise ValueError(f"Invalid source geometry: {name}/{feature['id']}")
                p = feature["properties"]
                values = {**p, "properties_json": p}
                if "Line" in g.geom_type:
                    values["geometry_miles"] = GEOD.geometry_length(g) / 1609.344
                if "Polygon" in g.geom_type:
                    values["geometry_area_acres"] = (
                        abs(GEOD.geometry_area_perimeter(g)[0]) / 4046.8564224
                    )
                insert(
                    db,
                    name,
                    {"geom": b"GP" + bytes([0, 1]) + struct.pack("<i", 4326) + g.wkb, **values},
                )
                ...
                insert(
                    ...
                    dict(
                        ...
                        geometry_sha256=hashlib.sha256(g.wkb).hexdigest(),
                        source_id=p["source_id"],
                        decision_id=p.get("decision_id"),
                        reference_source_ids=p.get("reference_source_ids", [p["source_id"]]),
                        construction_method=p.get(
                            "construction_method",
                            "Federal source service extraction; exact query and hashes retained.",
                        ),
                        reviewed_by="Agent structural validation; see reports",
                        review_date=catalog["world_state_date"],
            ...
            db.execute(
                ...
                (
                    ...
                    "features",
                    name,
                    "Controlled geographic layer; feature status governs use",
                    timestamp,
                    *extent,
                    4326,
                ),
        ...
        for name in [
            ...
            db.execute(
                "INSERT INTO gpkg_contents(table_name,data_type,identifier,last_change) VALUES(?,?,?,?)",
                (name, "attributes", name, timestamp),
            )
        for k, v in {
            "package_version": catalog["package_version"],
            "source_commit": catalog["source_commit"],
            "world_state_date": catalog["world_state_date"],
            "recorded_at": catalog["recorded_at"],
            "status": "CANON_RECONCILED_FRAMEWORK_ENGINEERING_INCOMPLETE",
            "source_input_sha256": digest(BASE / "sources/catalog.json"),
        }.items():
            insert(db, "package_metadata", dict(key=k, value=v))
...
def registers():
    ...
    write_csv(
        ...
        [
            ...
            if x["object_id"].startswith(("SH-SITE", "SH-FAC", "SH-BR"))
        ],
        list(catalog["objects"][0]),
    )
    write_csv(
        out / "PROVENANCE_COVERAGE.csv",
        [
            dict(
                ...
                status=x["canon_status"],
            )
            for x in catalog["objects"]
        ],
        ["object_id", "source_id", "source_path", "source_locator", "decision_id", "status"],
    )
    ...
    lines = [
        ...
        + "`. One row per adjudicated named object. City/corridor locks and geometry status remain separate. The full source-occurrence index also retains generic and generated references.",
        "",
    ...
    lines = [
        "# Open geographic questions v0.1",
        "",
        "Only material canon choices require the owner. Routine CRS, layout, schema, and envelope drafting are implementation work.",
    ]
    for c in catalog["conflicts"]:
        lines.extend(
            [
                "",
                f"## {c['conflict_id']} — {c['title']} [{c['status']}]",
                "",
                c["question"],
                "",
                "Recommendation / disposition: " + c["recommendation"],
                "",
                "Effect: " + c["implications"],
            ]
        )
    lines.extend(
        [
    ...
    for r in catalog["sources"] + [
        ...
        lines.append(
            "| "
            + " | ".join(esc(r[k]) for k in ["source_id", "title", "url_or_repo_path", "license"])
            + " |"
        )
...
def qgis_project(gpkg):
    ...
    catalog, refs = load_inputs()
    root = ET.Element(
        "qgis", version="3.28.0", projectname="Sable Harbor governed geographic framework"
    )
    ET.SubElement(root, "title").text = "SABLE HARBOR | CONSTRAINED GEOGRAPHIC FRAMEWORK"
    pcs = ET.SubElement(root, "projectCrs")
    srs = ET.SubElement(pcs, "spatialrefsys")
    ET.SubElement(srs, "authid").text = "EPSG:4326"
    ET.SubElement(srs, "description").text = "WGS 84"
    ET.SubElement(srs, "wkt").text = CRS.from_epsg(4326).to_wkt()
    canvas = ET.SubElement(root, "mapcanvas")
    extent = ET.SubElement(canvas, "extent")
    for k, v in zip(["xmin", "ymin", "xmax", "ymax"], [-125, 24, -66, 50]):
        ET.SubElement(extent, k).text = str(v)
    tree = ET.SubElement(root, "layer-tree-group", name="", checked="Qt::Checked", expanded="1")
    pls = ET.SubElement(root, "projectlayers")
    groups = {
        n: ET.SubElement(tree, "layer-tree-group", name=n, checked="Qt::Checked", expanded="1")
        for n in [
            "Governed search areas and disputed source claims",
            "Federal reference geography",
            "Unbuilt operational layers — zero features",
        ]
    }
    for name, features in layers(catalog, refs):
        id = "sh_" + name
        group = groups[
            ...
            if name.startswith("ref_")
                ...
                "Unbuilt operational layers — zero features"
                if not features
                else "Governed search areas and disputed source claims"
            )
        ]
        ...
        ET.SubElement(
            ...
            source=src,
            providerKey="ogr",
            checked="Qt::Checked" if features else "Qt::Unchecked",
            expanded="0",
        )
        ...
        for tag, text in [("id", id), ("datasource", src), ("layername", name)]:
            ET.SubElement(ml, tag).text = text
        ET.SubElement(ml, "provider", encoding="UTF-8").text = "ogr"
        layer_srs = ET.SubElement(ml, "srs")
        layer_srs.append(ET.fromstring(ET.tostring(srs)))

        def symbol(parent, index, color):
            stype = "fill" if geometry == "Polygon" else "marker" if geometry == "Point" else "line"
            sym = ET.SubElement(
                parent, "symbol", type=stype, name=str(index), alpha="1", clip_to_extent="1"
            )
            klass = {"fill": "SimpleFill", "marker": "SimpleMarker", "line": "SimpleLine"}[stype]
            sl = ET.SubElement(
                sym, "layer", **{"class": klass, "enabled": "1", "locked": "0", "pass": "0"}
            )
            options = {
                "color": color,
                "outline_color": color,
            ...
            if stype == "fill":
                options.update(
                    style="no", outline_style="dash" if name == "search_areas" else "solid"
                )
            elif stype == "marker":
                options.update(
                    name="cross2" if name == "source_claim_points" else "circle",
                    size="2.5",
                    size_unit="MM",
                )
            else:
        ...
        if name == "search_areas":
            renderer = ET.SubElement(
                ml, "renderer-v2", type="categorizedSymbol", attr="canon_status", symbollevels="0"
            )
            categories = ET.SubElement(renderer, "categories")
            symbols = ET.SubElement(renderer, "symbols")
            for i, (status, color) in enumerate(
                [("CANON_CONSTRAINED", "36,113,109,255"), ("CONFLICTING", "146,45,53,255")]
            ):
                ET.SubElement(
                    categories, "category", value=status, label=status, symbol=str(i), render="true"
                )
                symbol(symbols, i, color)
        else:
            renderer = ET.SubElement(ml, "renderer-v2", type="singleSymbol", symbollevels="0")
            symbols = ET.SubElement(renderer, "symbols")
            color = (
                "146,45,53,255"
                if name == "source_claim_points"
                else "97,148,175,255"
                if name.endswith("hydro")
                else "174,157,123,255"
                if name.endswith("highways")
                else "99,112,120,255"
                if "rail" in name
                else "185,190,188,255"
            )
    ...
    info = zipfile.ZipInfo("sable_harbor_master.qgs", (2026, 9, 6, 0, 0, 0))
...
def main():
    ...
    ap.add_argument("--database-only", action="store_true")
    args = ap.parse_args()
    p = build(args.output)
