...
ROOT = BASE.parent
CANON = "d91a22c35c91b213204411421de88815f27d8e16"
TERMS = re.compile(
    r"\b(location|site|facility|office|headquarters|HQ|mine|plant|yard|terminal|rail|railroad|warehouse|campus|lab|field|district|region|state|county|city|route|corridor|port|interchange|branch|project|acquisition|lease|property|parcel|Sacramento|Pittsburgh|Charleston|Belle|Kanawha|Hazelwood|Wamsutter|Bloodstone|Taylor|Bedford|Fairmont|Demotte|Klein|Fort|Rawlins|Nevada|Wyoming|Deloraine|Tasmania|Australia|Afghanistan|Badakhshan|Reno|Elko|Tucson|California|Pennsylvania|Arizona|Sar.e.Sang|Carbon|Sweetwater|Red.Wash|Blackridge|West.Highwall)\b",
    re.I,
)
TEXT_EXT = {
    ".md",
...
def extract(data, name, temp, ocr):
    ...
    if suffix in TEXT_EXT:
        return [
            (f"line:{i}", line) for i, line in enumerate(data.decode("utf-8").splitlines(), 1)
        ], "TEXT_SCANNED"
    if suffix == ".pdf":
    ...
    if suffix in {".docx", ".pptx", ".xlsx"}:
        ...
        with zipfile.ZipFile(io.BytesIO(data)) as z:
            for member in sorted(z.namelist()):
                if member.endswith(".xml") and member.startswith(
                    ("word/", "ppt/slides/", "ppt/notesSlides/", "xl/")
                ):
    ...
    if suffix in {".png", ".jpg", ".jpeg"}:
        ...
        return [
            (f"ocr-line:{i}", line) for i, line in enumerate(proc.stdout.splitlines(), 1)
        ], "IMAGE_OCR_NOT_VISUAL_VERIFICATION"
...
def main():
            ...
            for count, member in enumerate(members, 1):
                ...
                for locator, line in rows:
                    ...
                    if matches:
                        ...
                        hits.append(
                            {
                                "occurrence_id": "OCC-" + sha(key.encode())[:16],
                                "source_path": member.name,
                                "source_locator": locator,
                                "source_commit": commit,
                                "exact_source_wording": line,
                                "matched_terms": ";".join(matches),
                                "extraction_method": method,
                                "review_status": "DISCOVERY_OCCURRENCE_NOT_CANON",
                            }
                        )
                if count % 100 == 0:
    ...
    for hit in hits:
        ...
        if wording not in unique:
            unique[wording] = {
                "statement_id": "STMT-" + sha(wording.encode())[:16],
                "exact_source_wording": wording,
                "occurrence_count": 0,
                "example_source": hit["source_path"],
                "example_locator": hit["source_locator"],
                "review_status": "DISCOVERY_ONLY_SEE_CURATED_OBJECT_REGISTER",
            }
    ...
    manifest = {
        "source_commit": commit,
        "tracked_files": len(coverage),
        "candidate_occurrences": len(hits),
        "extraction_methods": dict(Counter(r["extraction_method"] for r in coverage)),
        "errors": errors,
        "semantic_census_complete": False,
        "scope": "Entire pinned main tree; text, Office XML, PDF text, SQLite and ZIP contents extracted. Raster extraction status is explicit per file; no blanket visual-review claim.",
        "ocr_enabled": not args.no_ocr,
        "canon_promotion": "None. Adjudicated objects live in GEOGRAPHIC_CENSUS_v0.1.csv.",
    }
