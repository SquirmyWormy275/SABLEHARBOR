...
with sqlite3.connect(gpkg) as db:
    names = [
        r[0] for r in db.execute("SELECT table_name FROM gpkg_contents WHERE data_type='features'")
    ]
...
with tempfile.TemporaryDirectory(prefix="sh-qgis-relocation-") as temp:
    ...
    for name, path in [
        ...
        ("relocated", moved / "qgis" / source_project.name),
    ]:
        ...
        for layer in sorted(project.mapLayers().values(), key=lambda x: x.name()):
            ...
            layers.append(
                dict(
                    ...
                    valid=layer.isValid(),
                    feature_count=count,
                    expected_count=expected.get(layer.name()),
        ...
        passed = (
            read
            and len(layers) == len(expected)
            and all(
                entry["valid"]
                and entry["feature_count"] == entry["expected_count"]
                and entry["crs"] == "EPSG:4326"
                and (entry["renderer"] or entry["feature_count"] == 0)
                for entry in layers
            )
        )
        checks.append(
