"""Reproducible spatial addendum with public source context and offline atlas."""
...
def package(revision, output):
    ...
    with (
        ...
        for item in sorted(archive.getmembers(), key=lambda p: p.name):
            ...
            if item.name == "docs/releases/FACILITY_SPATIAL_RELEASES.md":
                content = (
                    f"# Spatial delivery\n\nSource `{revision}`. Open [the spatial review](../../geospatial/maps/spatial.html).\n\nSee [embedded hashes](../../SPATIAL_PACKAGE_MANIFEST.json) and [published acceptance and checksums](https://github.com/SquirmyWormy275/SABLEHARBOR/releases/tag/facility-spatial-v1.0.0). This navigation avoids circular package checksums.\n"
                ).encode()
        ...
        manifest = {
            "package": "SABLE_HARBOR_Facility_Spatial_v1.0.0",
            "source_commit": revision,
            "accepted_facility_base": "7bc9879fb94dbf999066b8072cc66ec8e05fda0a",
            "entrypoint": "geospatial/maps/spatial.html",
        ...
        write(bundle, "SPATIAL_PACKAGE_MANIFEST.json", content)
