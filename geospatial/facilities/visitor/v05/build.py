...
from r01_drawing import Sheet, INK, MUTED, PAPER, GOLD, FT  # noqa: E402 — local renderer path configured above
...
def build():
    ...
    def xy(x, y, z=0):
        x -= projection["center_ft"][0]
        y -= projection["center_ft"][1]
        return (
            projection["origin"][0] + x * projection["east"][0] + y * projection["north"][0],
            projection["origin"][1]
            + x * projection["east"][1]
            + y * projection["north"][1]
            + z * projection["up"][1],
        )

    def polygon(points, fill, stroke="none", width=1):
        s.path(
            "M" + " L".join(f"{x:.2f},{y:.2f}" for x, y in [xy(*p) for p in points]) + " Z",
            fill,
            stroke,
            width,
        )
    ...
    def label(x, y, text, size=23, color=INK):
        ...
        s.parts.insert(
            len(s.parts) - 1,
            s.parts[-1].replace(
                "<text ", '<text stroke="' + PAPER + '" stroke-width="6" stroke-linejoin="round" '
            ),
        )
    ...
    for r in drawing["walks_ft"]:
        rect(r, "#fafaf6", "#d2d8cd", 0.9)
    rect(drawing["service_apron_ft"], "#dce1d7")
    for lot in drawing["parking_blocks"]:
        x, y, w, h = lot["rect_ft"]
        n = lot["columns"]
        rect(lot["rect_ft"], "#e2e6de", "#b8c3b5", 0.9)
        for i in range(n + 1):
            if lot["vertical"]:
                for xx in [x, x + w - 18]:
                    line((xx, y + i * h / n), (xx + 18, y + i * h / n), "#b7c2b2", 0.8)
            else:
                for yy in [y, y + h - 18]:
                    line((x + i * w / n, yy), (x + i * w / n, yy + 18), "#b7c2b2", 0.8)
        label(x + w / 2, y + h / 2, "P", 24)
    for c in drawing["courts"]:
        rect(c["rect_ft"], "#dce5d4", "#c3ceb9", 0.8)
    ...
    s.parts.append(
        f'<path d="{d}" fill="none" stroke="{GOLD}" stroke-width="7" stroke-linecap="round" stroke-linejoin="round"/>'
    )
    ...
    objects += [
        (
            "building",
            b,
            xy(b["rect_ft"][0] + b["rect_ft"][2] / 2, b["rect_ft"][1] + b["rect_ft"][3] / 2)[1],
        )
    ...
    for kind, obj, _ in sorted(objects, key=lambda o: o[2]):
        if kind == "tree":
            ...
            s.parts.append(
                f'<ellipse cx="{px + 6:g}" cy="{py + 3:g}" rx="16" ry="7" fill="#b8c6b2" opacity=".4"/>'
            )
            s.line(px, py, px, py - 20, "#82917a", 2)
            s.parts.append(
                f'<ellipse cx="{px:g}" cy="{py - 20:g}" rx="17" ry="21" fill="#bdcdb4" stroke="#a7bba0" stroke-width=".8"/>'
            )
            s.parts.append(
                f'<ellipse cx="{px - 4:g}" cy="{py - 23:g}" rx="10" ry="14" fill="#ccdac5"/>'
            )
            continue
        raw = obj
        b = next(b for b in site["buildings"] if b["id"] == raw["id"])
        x, y, w, h = raw["rect_ft"]
        height = b["height_m"] / FT
        top = (b["height_m"] + b["parapet_m"]) / FT
        ...
        for edge, span in [("south", b["rect_m"][2]), ("west", b["rect_m"][3])]:
            ...
            for floor in b["floors"]:
                z = floor["z_m"]
                sill = z + assumptions["facade_glazing_sill_m"]
                head = z + floor["height_m"] - assumptions["facade_glazing_head_clearance_m"]
                for j in range(count):
                    ...
                    for core in floor["cores"]:
                        ...
                        if touch and max(a, lo) < min(end, hi):
                            blocked = True
                    if blocked:
                        continue
                    if edge == "south":
                ...
                if edge == "south":
                    line((x, y, z / FT), (x + w, y, z / FT), "#b4c1bb", 1)
                else:
                    line((x, y, z / FT), (x, y + h, z / FT), "#9dafaa", 1)
        rect([x, y, w, h], "#f8faf7", "#718a86", 1.3, z=top)
        # Parapet from the existing study; roof remains free of invented equipment.
        rect([x + 1.5, y + 1.5, w - 3, h - 3], "#e6ece7", "#b7c6bb", 0.8, z=height)
        ...
        entry = [
            v / FT
            for v in campus["access"][source["building_labels"][raw["r01_letter"]]["entry_key"]][-1]
        ]
        entries.append(
            {
                "building_id": b["id"],
                "entry_ft": entry,
                "source_access_key": source["building_labels"][raw["r01_letter"]]["entry_key"],
            }
        )
        masses.append(
            {
                "building_id": b["id"],
                "ground_rect_ft": raw["rect_ft"],
                "height_m": b["height_m"],
                "parapet_m": b["parapet_m"],
                "modelled_floors": len(b["floors"]),
            }
        )
    # Ground-level entrance markers and labels stay readable after the architectural pass.
    for e in entries:
        px, py = xy(*e["entry_ft"])
        s.circle(px, py, 8, INK, PAPER, 3)
    ...
    dx, dy, dw, dh = next(
        z["rect_ft"] for z in campus["site_zones"] if z["name"] == "Drop-off lane"
    )
    label(dx + dw / 2, dy + dh / 2, "Drop-off", 22, MUTED)
    px, py = route[0]
    s.circle(px, py, 10, GOLD, PAPER, 3)
    ...
    s.path(
        f"M{tx},{ty} L{tx - vx * 20 - vy * 8},{ty - vy * 20 + vx * 8} L{tx - vx * 20 + vy * 8},{ty - vy * 20 - vx * 8} Z",
        INK,
        "none",
    )
    ...
    for i, (letter, name, description) in enumerate(directory):
        x = 130 + i * 755
        s.circle(x + 22, 1810, 22, INK, "none")
        s.text(x + 22, 1819, letter, 25, fill="white", weight="bold", anchor="middle")
        s.text(x + 68, 1821, name, 38, weight="bold")
        s.text(x + 68, 1870, description, 25, fill=MUTED)
    ...
    s.line(1580, 1984, 1630, 1984, GOLD, 7)
    s.text(1660, 1994, "Path to Corporate reception", 25)
    s.circle(1605, 2050, 7, INK, "none")
    s.text(1660, 2060, "Building entrance", 25)
    s.text(2220, 1994, "P", 25, weight="bold")
    s.text(2270, 1994, "Parking area", 25)
    ...
    s.text(
        130,
        2253,
        "Architecture follows the saved concept study; materials and facade design remain unselected.",
        21,
        fill=MUTED,
    )
    ...
    result.update(
        ...
        geometry_changes=[],
        entrances=entries,
        building_masses=masses,
        facade_openings=openings,
    ...
    dependencies = [
        ...
        ROOT / source["spatial_source"],
        ROOT / source["architectural_assumptions"],
        *sorted((FAC / "fonts").glob("*")),
        *[ROOT / p for p in campus["approved_reference_sha256"]],
    ]
    ...
    print(
        f"V05: {len(masses)} source-based buildings, {len(openings)} existing-study facade openings; earlier drafts preserved."
    )
