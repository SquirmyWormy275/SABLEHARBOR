...
assert walks.buffer(0.001).covers(route)
assert len(manifest["entrances"]) == len(manifest["building_masses"]) == 4
for raw in model["buildings"]:
    x, y, w, h = raw["rect_ft"]
    assert route.intersection(box(x, y, x + w, y + h)).length < 0.001
    ...
    assert entry["entry_ft"] == [
        v / 0.3048 for v in model["access"][entry["source_access_key"]][-1]
    ]
...
for b in site["buildings"]:
    for edge, span in [("south", b["rect_m"][2]), ("west", b["rect_m"][3])]:
        n = max(1, round(span / a["facade_target_bay_m"]))
        bay = span / n
        for f in b["floors"]:
            blocked_intervals = []
            for c in f["cores"]:
            ...
            for j in range(n):
                start, end = (j + 0.16) * bay, (j + 0.84) * bay
                if any(max(start, lo) < min(end, hi) for lo, hi in blocked_intervals):
                    continue
                expected.append(
                    (
                        b["id"],
                        ...
                        f["z_m"] + a["facade_glazing_sill_m"],
                        f["z_m"] + f["height_m"] - a["facade_glazing_head_clearance_m"],
                    )
                )
actual = [
    (r["building_id"], r["floor_id"], r["edge"], *r["span_m"], *r["z_m"])
    for r in manifest["facade_openings"]
]
    ...
    for block in page.get_text("dict")["blocks"]:
        for line in block.get("lines", []):
            for span in line["spans"]:
                assert page.rect.contains(fitz.Rect(span["bbox"])), span["text"]
    for text in [
        "Sacramento",
        ...
        "Building entrance",
        "Parking area",
        "facade design remain unselected",
    ]:
...
print(
    f"PASS V05: 4 source footprints/stacks; {len(actual)} core-aware study openings; route; artifacts; text bounds; links; V02 preservation"
)
