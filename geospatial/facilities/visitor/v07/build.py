...
pdf.set_metadata({'title':'Sable Harbor visitor illustration V07 — not a metric plan','author':'Sable Harbor','creationDate':'D:20260911000000Z','modDate':'D:20260911000000Z'})
...
manifest={'revision':'V07','status':source['status'],'map_id':None,'metric_geometry_validated':False,'observed_deviations':source['observed_deviations'],'sources':{str(p.relative_to(ROOT)):sha(p) for p in paths},'artifacts':{ext:{'path':str((BASE/f'artifacts/visitor-map-v07.{ext}').relative_to(ROOT)),'sha256':sha(BASE/f'artifacts/visitor-map-v07.{ext}')} for ext in ['png','pdf']}}
