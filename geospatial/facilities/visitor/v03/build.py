...
def build():
    ...
    s=Sheet(source['title'],source['subtitle'],source['site_id'],'VISITOR GUIDE',revision='V03')
    ...
    s.parts=[part.replace('>CAMPUS VISITOR GUIDE</text>', '>Campus visitor guide</text>').replace('y="249" font-size="62.5"', 'y="249" font-size="76"').replace('y="249" font-size="76" fill="#223740" font-weight="bold"', 'y="249" font-size="76" fill="#223740" font-weight="normal"') for part in s.parts]
    ...
    for r in drawing['roads_ft']:rect(r,'#ccc9c0')
    for r in drawing['walks_ft']:rect(r,'#f0e7d4','#d2c4a8',1)
    rect(drawing['service_apron_ft'],'#ddd5c5','#b6beb5',1)
    for lot in drawing['parking_blocks']:
        x,y,w,h=lot['rect_ft'];n=lot['columns'];rect(lot['rect_ft'],'#dcdad0','#b0b1a3',1.5)
        for i in range(n+1):
            if lot['vertical']:
                for xx in [x,x+w-18]:s.line(*xy(xx,y+i*h/n),*xy(xx+18,y+i*h/n),'#b4b6a9',1.2)
            else:
                for yy in [y,y+h-18]:s.line(*xy(x+i*w/n,yy),*xy(x+i*w/n,yy+18),'#b4b6a9',1.2)
        if lot['vertical']:
            for xx in [x+18,x+w-18]:s.line(*xy(xx,y),*xy(xx,y+h),'#b4b6a9',1.2)
        else:
            for yy in [y+18,y+h-18]:s.line(*xy(x,yy),*xy(x+w,yy),'#b4b6a9',1.2)
    ...
    for court in drawing['courts']:
        x,y,w,h=court['rect_ft'];rect(court['rect_ft'],'#e9ddc5','#c7bda6',1.2)
    ...
    for i,(x,y) in enumerate(drawing['trees_ft']):
        ...
        def crown(cx,cy):
            ...
            for j in range(48):
                ...
                r=radius*(1+.065*math.sin(angle*7+i*.8)+.035*math.cos(angle*5+i))
        ...
        s.path(f'M{px-12},{py+3} Q{px-16},{py-13} {px+3},{py-15}',stroke='#c2ccad',width=1.5)
    entries=[]
    for b in model['buildings']:
        ...
        s.text(*xy(x+w/2,y+h/2-9),(spec['name'].title() if letter!='B' else 'J2'),38,weight='normal',anchor='middle')
    ...
    result=s.save(BASE/'artifacts','visitor-map-v03',{'title':source['title'],'site_id':source['site_id'],'status':source['status'],'map_id':None,'production_allocation':'PENDING_VISUAL_ACCEPTANCE'})
    result['review_surface']={'path':str((BASE/'review.html').relative_to(ROOT)),'sha256':sha(BASE/'review.html')}
    result['revision']='V03';result['geometry_changes']=[];result['entrances']=entries
