...
def build():
    ...
    s = Sheet(
        source["title"], source["subtitle"], source["site_id"], "VISITOR GUIDE", revision="V04"
    )
    s.parts = [
        p.replace(
    ...
    for r in drawing["walks_ft"]:
        rect(r, "#f9f8f3", "#c7cabe", 1)
    rect(drawing["service_apron_ft"], "#e0e2da", "#b6beb5", 1)
    for lot in drawing["parking_blocks"]:
        x, y, w, h = lot["rect_ft"]
        n = lot["columns"]
        rect(lot["rect_ft"], "#e1e4dc", "#9aa69a", 1.5)
        for i in range(n + 1):
            if lot["vertical"]:
                for xx in [x, x + w - 18]:
                    s.line(*xy(xx, y + i * h / n), *xy(xx + 18, y + i * h / n), "#a7b0a6", 1.2)
            else:
                for yy in [y, y + h - 18]:
                    s.line(*xy(x + i * w / n, yy), *xy(x + i * w / n, yy + 18), "#a7b0a6", 1.2)
        if lot["vertical"]:
            for xx in [x + 18, x + w - 18]:
                s.line(*xy(xx, y), *xy(xx, y + h), "#a7b0a6", 1.2)
        else:
            for yy in [y + 18, y + h - 18]:
                s.line(*xy(x, yy), *xy(x + w, yy), "#a7b0a6", 1.2)
    ...
    for court in drawing["courts"]:
        ...
        rect(court["rect_ft"], "#e4e9dd", "#bac4b5", 1.2)
        name = {
        ...
        s.text(
            *xy(x + w / 2, y + h / 2 - 2), name, 21, fill="#657567", tracking=1.0, anchor="middle"
        )
    for x, y in drawing["trees_ft"]:
        px, py = xy(x, y)
        s.circle(px, py, 21, "#d7e0ce", "#aebca5", 1.4)
        s.circle(px, py, 15, "none", "#c2ceb7", 0.8)
    entries = []
    masses = []
    for b in model["buildings"]:
        ...
        def face(points, fill):
            s.path(
                "M" + " L".join(f"{xx:g},{yy:g}" for xx, yy in [xy(*pt) for pt in points]) + " Z",
                fill,
                "#8b9da0",
                1.3,
            )
        ...
        for level in range(1, b["modelled_floor_count"]):
            ...
            s.line(
                *xy(x + shift * t, y + rise * t),
                *xy(x + w + shift * t, y + rise * t),
                "#dce4e3",
                1.1,
            )
        ...
        label = spec["name"]
        s.text(
            *xy(x + shift + w / 2, y + h * 0.45 + rise), label, 33, weight="bold", anchor="middle"
        )
        cx, cy = xy(x + shift + w / 2, y + h * 0.45 + rise + 19)
        s.circle(cx, cy, 21, INK, "none")
        s.text(cx, cy + 9, letter, 25, fill="white", weight="bold", anchor="middle")
        masses.append(
            {
                "building_id": b["id"],
                "ground_rect_ft": b["rect_ft"],
                "modelled_floors": b["modelled_floor_count"],
                "height_ft": height_ft,
                "roof_offset_ft": rise,
                "roof_east_offset_ft": shift,
            }
        )
    ...
    s.text(
        1470,
        2113,
        "GROUND-PLANE SCALE  /  NORTH IS UP",
        21,
        fill=MUTED,
        tracking=1,
        anchor="middle",
