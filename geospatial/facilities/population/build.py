...
def build():
    ...
    for node in chart['nodes']:
        ...
        if pid not in by_person:
            by_person[pid] = {'person_id': pid, 'name': node['name'], 'status': node['status'],
                              'display_ids': [], 'sources': [], 'joining_records': [], 'counting_weight': 1,
                              'workplace_assignment': None, 'workplace_assignment_status': 'NOT_ESTABLISHED_BY_PERSON_DISPLAY'}
        person = by_person[pid]
        assert person['status'] == node['status'], f'conflicting status {pid}'
        person['display_ids'].append(node['id'])
        person['joining_records'].append({'display_id': node['id'], 'year': node.get('joined_year'), 'basis': node.get('year_basis'), 'not_an_occupancy_date': True})
        for source in node['sources']:
            if source not in person['sources']:
                person['sources'].append(source)
    people = sorted(by_person.values(), key=lambda x: x['person_id'])
    excluded = [{k: n[k] for k in ('id', 'name', 'status', 'sources')} for n in chart['register_only']
                if n['id'].startswith('P') and n['id'][1:].isdigit()]
    groups = copy.deepcopy(policy['j2_billet_groups'])
    for group in groups:
        group.update(current_named_employees=len(group['named_person_ids']),
                     unnamed_authorized_billets=group['authorized_billets']-len(group['named_person_ids']),
                     confirmed_vacancies=None, actual_occupied_billets=None,
                     source=paths['establishment'], status='LOCKED_AUTHORIZATION_NOT_OCCUPANCY')
    ...
    for node in chart['nodes'] + chart['register_only']:
        if node.get('type') == 'person' or (node['id'].startswith('P') and node['id'][1:].isdigit()):
            continue
        units.append({'id': node['id'], 'name': node['name'], 'status': node['status'],
                      'sources': node['sources'], 'unit_kind': 'role_record' if node['id'].startswith('ROLE-') else 'entity_or_function',
                      'current_named_employees': None, 'authorized_positions': None,
                      'confirmed_vacancies': None, 'remote_distributed_field_deployed': None,
                      'resident_occupants': None, 'visitors_customers_trainees_peak': None,
                      'shift_population': None, 'maximum_concurrent_attendance': None,
                      'seat_assignment_source': 'geospatial/facilities/source/campus.json',
                      'assigned_desks': None, 'shared_desks': None, 'touchdown_seats': None,
                      'training_seats': None, 'meeting_seats': None, 'special_use_capacity': None,
                      'horizons': [{'year': y, 'proposed_future_positions': None,
                                    'status': 'UNRESOLVED_NO_EMPLOYMENT_CENSUS' if y == 2026 else 'UNRESOLVED_PLANNING_CAPACITY',
                                    'net_assignable_area_m2': None} for y in policy['horizons']],
                      'rollup_rule': 'NONADDITIVE_VIEW; normalized people and J2 groups control counts, site program controls physical capacity'})
    return {'record_id': policy['record_id'], 'as_of': policy['as_of'], 'schema_version': '1.0.0',
            'source_revision': {'chart_publication': chart['publication_revision'], 'chart_as_of': chart['as_of'],
                                'source_sha256': {p: hashlib.sha256((ROOT/p).read_bytes()).hexdigest() for p in sorted(set(paths.values()) | {str((HERE/'policy.json').relative_to(ROOT))})}},
            'people': people, 'excluded_register_people': excluded, 'entities_functions_roles': units,
            'j2': {'authorized_billets': sum(g['authorized_billets'] for g in groups),
                   ...
                   'confirmed_vacancies': None, 'actual_occupied_billets': None, 'groups': groups},
            'industrial': {'status': 'ACCEPTED_SELECTED_OPERATING_CASE_NOT_SITE_ATTENDANCE', 'source': paths['industrial'],
                           'values': industrial, 'selected_population_total': industrial['pale_sun_red_wash_total']+industrial['aru_consolidated'],
                           'actual_concurrent_attendance': None},
            'conditional_forecast': {'year': forecast['start_year'], 'status': forecast['fact_state'], 'source': paths['forecast'],
                                     'groups': forecast['workforce'], 'occupied': sum(x['occupied'] for x in forecast['workforce'].values()),
                                     'authorized': sum(x['authorized'] for x in forecast['workforce'].values())},
            'totals': {'person_display_records': sum(len(p['display_ids']) for p in people), 'unique_display_people': len(people),
                       'current_named_employees': sum(p['status']=='current_employee' for p in people),
                       'current_named_nonemployee_directors': sum(p['status']=='current_nonemployee_director' for p in people),
                       ...
                       'excluded_register_people': len(excluded), 'actual_company_headcount': None,
                       'actual_company_peak_attendance': None, 'actual_company_seats': None},
            'measurement_rules': policy['measurement_rules'], 'planning_basis': policy['planning_basis']}
...
def validate(data):
    ...
    people = data['people']; totals = data['totals']; j2 = data['j2']; groups = j2['groups']
    for collection, key in [(people, 'person_id'), (data['entities_functions_roles'], 'id'), (groups, 'id')]:
        ids = [x[key] for x in collection]; check(len(ids)==len(set(ids)), f'duplicate {key}')
    check(all(p['counting_weight']==1 for p in people), 'person counted fractionally or more than once')
    ...
    for status, key in [('current_employee', 'current_named_employees'), ('current_nonemployee_director', 'current_named_nonemployee_directors'), ('former_employee', 'former_display_people')]:
        check(sum(p['status']==status for p in people)==totals[key], f'named rollup {key}')
    check(totals['current_named_people']==totals['current_named_employees']+totals['current_named_nonemployee_directors'], 'named people/director conflation')
    ...
    check(all(pid in {p['person_id'] for p in people if p['status']=='current_employee'} for pid in ids), 'J2 person not current employee')
    ...
    for g in groups:
        ...
        check(expected is not None and int(expected.group(1)) == g['authorized_billets'], f'J2 source authorization {g["id"]}')
        check(g['current_named_employees']==len(g['named_person_ids']), f'J2 named allocation {g["id"]}')
        check(g['current_named_employees']+g['unnamed_authorized_billets']==g['authorized_billets'], f'J2 horizontal rollup {g["id"]}')
        check(g['confirmed_vacancies'] is None and g['actual_occupied_billets'] is None, 'unnamed billets conflated with vacancy/occupancy')
    check(j2['authorized_billets']==237, 'J2 locked 237 changed')
    for key in ['actual_company_headcount', 'actual_company_peak_attendance', 'actual_company_seats']:
        check(totals[key] is None, f'unsupported actual total {key}')
    f = data['conditional_forecast']
    check(f['year']==2027 and f['status']=='CONDITIONAL_FORECAST', 'forecast promoted into actuals')
    ...
    check(v['pale_sun_platform']+v['red_wash_site']==v['pale_sun_red_wash_total'], 'Pale Sun overlap')
    check(v['aru_nonrail_and_corporate']+v['bst']==v['aru_consolidated'], 'ARU overlap')
    check(v['pale_sun_red_wash_total']+v['aru_consolidated']==data['industrial']['selected_population_total'], 'industrial rollup')
    for path, digest in data['source_revision']['source_sha256'].items():
        check((ROOT/path).exists() and hashlib.sha256((ROOT/path).read_bytes()).hexdigest()==digest, f'stale source {path}')
    return errors

def bridge(data):
    t=data['totals']; lines=['# Headcount, occupancy and space bridge', '',
    ...
    for g in data['j2']['groups']:
        lines.append(f"| {g['name']} | {g['authorized_billets']} | {g['current_named_employees']} | {g['unnamed_authorized_billets']} | Unknown |")
    lines += ['', 'Every imported numeric source is linked and checksummed in REGISTER.json. `DISCREPANCY_REPORT.json` is generated only after horizontal and vertical checks; `--check` fails on source drift, duplicate identities, inconsistent totals, or a stale register/bridge/report.', '']
    return '\n'.join(lines)
...
def outputs(data):
    ...
    if errors:
        raise ValueError('\n'.join(errors))
    return {'REGISTER.json': json.dumps(data, indent=2, ensure_ascii=False)+'\n',
            'BRIDGE.md': bridge(data),
            'DISCREPANCY_REPORT.json': json.dumps({'record_id':data['record_id'], 'status':'PASS', 'errors':[], 'totals':data['totals'], 'checks':['identity deduplication','J2 horizontal and vertical rollups','industrial hierarchy','conditional forecast isolation','source checksums','unknown actual measures preserved']},indent=2)+'\n'}
