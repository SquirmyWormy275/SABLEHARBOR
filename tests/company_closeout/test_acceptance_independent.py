...
def fixture(tmp_path, monkeypatch):
    ...
    scope = dict(
        ...
        adopted_sources=[
            dict(
                path="records.json",
                sha256=edition.sha((root / "records.json").read_bytes()),
    ...
    subprocess.run(
        ...
        env={
            **os.environ,
            "GIT_AUTHOR_DATE": "2026-09-22T06:00:00Z",
            "GIT_COMMITTER_DATE": "2026-09-22T06:00:00Z",
        },
    )
    ...
    contract = dict(
        ...
        scope="Bounded synthetic fixture",
        limitations=["No blanket control acceptance"],
        allowed_joins=["id"],
        required_components=["source"],
        components=[
            dict(
                id="source",
                fact_status="MIXED_SOURCE_ARCHIVE",
                access_scope="PUBLIC_SYNTHETIC",
                population_definition="Two source files",
                units=["records"],
                legal_entities=["SHI"],
                available_at="2026-09-22T10:00:00Z",
                members=[
                    dict(path=p, sha256=edition.sha((root / p).read_bytes()))
