...
def test_exact_approved_names_offices_and_company_joining_years():
    data = roster()
    assert [
        (p["person_id"], p["role_id"], p["name"], p["title"], p["joined_year"])
...
def test_existing_202_display_records_unchanged_and_only_six_people_added():
    ...
    for person_id, role_id, name, title, year in EXPECTED:
        item = new[person_id]
        assert (item["role_id"], item["name"], item["title"], item["joined_year"]) == (
            role_id,
            name,
            title,
            year,
...
def test_resolved_roles_preserve_original_ids_and_evidence():
    ...
    assert {r["id"]: r for r in roster()["resolved_role_records"]} == {
        key: old[key] for key in resolved
    }
...
def test_establishment_and_financial_authority_not_expanded():
    ...
    assert charts["people-enterprise"]["node_ids"] == old_charts["people-enterprise"][
        "node_ids"
    ] + ["P063"]
...
def test_archived_master_and_source_are_exact_reviewed_bytes():
    ...
    assert (
        digest(ORG / "history/v1.0.0/Sable-Harbor-Organization-Charts.pdf")
    ...
    assert (
        ...
        == "6cf1989a316f77e68e4c09720c56b9a5e289c17c"
    )


def test_j2_page_cards_and_six_borders_match_source():
    spec = importlib.util.spec_from_file_location(
        "export_charts_j2_test", ROOT / "tools/organization/export_charts.py"
    )
    ...
    with fitz.open(ROOT / data["visual_master"]) as pdf:
        ...
        rectangles = [
            ...
            if item[0] == "re" and drawing["type"] == "fs"
        ]
        for card in chart["pages"][0]["cards"]:
            assert module.card_fields(page, card["bounds"], True) == module.display_fields(
                nodes[card["node_id"]]
            )
            assert any(
                all(abs(a - b) < 0.01 for a, b in zip(rect, card["bounds"], strict=True))
...
def test_original_56_page_content_visually_preserved_outside_authorized_changes():
    with (
        fitz.open(ORG / "history/v1.0.0/Sable-Harbor-Organization-Charts.pdf") as old,
        fitz.open(ROOT / source()["visual_master"]) as current,
    ):
        ...
        for index in range(56):
            ...
            if index == 40:
                clips = [
                    fitz.Rect(0, 0, 1440, 638),
                    fitz.Rect(0, 780, 1440, 1030),
                    fitz.Rect(0, 638, 507, 780),
                    fitz.Rect(933, 638, 1440, 780),
                ]
            for clip in clips:
                assert (
                    old[index].get_pixmap(matrix=fitz.Matrix(0.4, 0.4), clip=clip).samples
                    == current[index].get_pixmap(matrix=fitz.Matrix(0.4, 0.4), clip=clip).samples
                ), (index + 1, clip)
