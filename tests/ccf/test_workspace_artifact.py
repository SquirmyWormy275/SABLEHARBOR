...
@pytest.mark.parametrize(
    "action", ["promote", "authoritative_write", "institutional_judgment", "train_shared"]
)
...
def test_revocation_owner_and_stale_source_denial(case):
    ...
    with pytest.raises(ValueError, match="isolation"):
        read(
            path, records, dict(subject, id="CEO"), NOW, expected_sha256=receipt["artifact_sha256"]
        )
    with pytest.raises(ValueError, match="denied"):
        read(
            path,
            records,
            dict(subject, revoked=True),
            NOW,
