...
def order():
    return {'order_id': 'TEST-ONLY', 'instrument_id': 'SH-CRADLE-KGM-B-20260922',
            'start': '2026-10-01', 'end': '2026-12-29', 'SH_acceptor': 'P025',
            'host_acceptor': 'SYN-HOST-KGM-REP-001', 'acceptance_received_at': '2026-09-30T12:00:00+00:00',
            'scope': 'Designated test-only run', 'interface_revision': 'S17-A', 'sale_capable': False,
            'economics': {'eligible_proceeds': 'None; test-only no-sale run', 'deductions': 'None',
                          'currency': 'USD', 'host_payment_formula': 'Explicit zero for this test fixture',
...
@pytest.mark.parametrize('key,value,message', [
    ('start', '2026-09-14', 'backdated'), ('host_acceptor', 'P025', 'authority'),
...
def test_blank_economics_not_zero_and_demotte_requires_title():
    ...
    with pytest.raises(ValueError, match='economics'):
        validate(data)
    row = order()
    row.update(instrument_id='SH-CRADLE-DEMOTTE-B-20260922', host_acceptor='SYN-HOST-DEMOTTE-REP-001', sale_capable=True)
    data['accepted_work_orders'] = [row]
...
def test_notice_runs_from_receipt_and_no_duplicate_orders():
    data = source()
    row = order()
    row.update(exit_notice_received_at='2026-10-01T12:00:00+00:00', early_exit_at='2026-10-31T11:59:59+00:00')
    data['accepted_work_orders'] = [row]
