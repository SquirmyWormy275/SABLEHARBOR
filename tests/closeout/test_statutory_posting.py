...
def test_acquired_dta_successor_hits_expense_not_opening_equity():
    ...
    p.annual_deferred = defaultdict(lambda: defaultdict(D))
    p.opening_deferred = defaultdict(lambda: defaultdict(D))
    b = Books('base', {'segment_mapping': {}, 'knowledge_cutoff': '2026-09-22', 'created_on': '2026-09-22'},
              TYPES | {'1150': 'asset', '2150': 'liability', '1000': 'asset', '3100': 'equity', '1800': 'asset'})
...
def inputs():
    ...
    def tax(s, e, j, y):
        return dict(scenario=s, taxpayer=e, jurisdiction=j, year=y, current_tax_usd='0', gross_dta_usd='0', valuation_allowance_usd='0', gross_dtl_usd='0')
    states = [tax(s,e,j,y) for s in scenarios for e in ('SHI','SHIH','PS','ARU','BST') for j in ('CA','IL','WV') for y in years]
    federal = [tax(s,e,'US',y) for s in scenarios for e in ('SHIH','PS','ARU','BST') for y in years]
    deferred = states + [tax(s,e,'US',y) for s in scenarios for e in ('SHI','PS','ARU','BST') for y in years]
    opening = [tax(s,e,j,2026) for s in scenarios for e,j in [('SHI','US'),('PS','US'),('SHI','CA'),('PS','IL'),('SHI','WV')]]
    parent = SimpleNamespace(rows=[dict(scenario=s,year=y) for s in scenarios for y in years])
    journal = [dict(scenario=s,entity=e,year=y,month=m,source_id='LEGAL-test',journal_id=f'{e}-{y}-{m}',line_no=1) for s in scenarios for e in ('ARU','BST','PS','RWH') for y in years for m in range(1,13)]
    settlement = dict(rows=[dict(scenario=s,source_group=g,year=y,month=m,gross_source_tax_cash_paid_usd='0') for s in scenarios for g in ('ARU_GROUP','RWH_PS') for y in years for m in range(1,13)], payments=[])
    return [dict(journal_rows=journal), parent, dict(federal=federal,states=states), deferred, opening, settlement]
...
@pytest.mark.parametrize('position,key', [(2,'federal'), (2,'states'), (5,'rows'), (0,'journal_rows')])
