...
import pytest
from enterprise.closeout.treasury import spread, build
from industrial.planning.enterprise import write_csv, read_csv


def fixture(tmp_path):
    root=tmp_path/'enterprise';root.mkdir()
    rows=[];statements=[]
    for entity in ['SHI','CONSOLIDATED']:
        for month in [8,9,10,11]:
            statements.append(dict(scenario='base',entity=entity,year=2026,month=month,
              ending_cash_usd=str(10+300*(month-8))))
    for month in [9,10,11]:
        rows.append(dict(scenario='base',entity='SHI',year=2026,month=month,account='1000',
          journal_id=f'J{month}',line_no=month,source_id=f'M{month}',source_type='MEMBER_EQUITY',
          cash_flow='FINANCING',signed_usd='300',available_at='2026-09-15',fact_state='CONDITIONAL_FORECAST'))
...
def test_thirteen_weeks_bridge_and_no_available_funding(tmp_path):
    fixture(tmp_path);result=build(tmp_path)
