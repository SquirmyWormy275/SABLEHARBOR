...
from enterprise.closeout.tax_calendar import build, return_due
...
def test_tax_and_legal_forms_do_not_collapse_or_invent_submissions():
    rows = build()
    assert len({r["filing_id"] for r in rows}) == len(rows)
    assert all(
        r["form"] == "LLC300" for r in rows if r["entity"] == "SHI" and r["jurisdiction"] == "DE"
    )
