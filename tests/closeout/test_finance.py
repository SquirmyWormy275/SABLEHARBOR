...
def books(scenario):
    b = Books(scenario, {'segment_mapping': {'FOUNDRY_FIELD':'foundry-field'},
...
@pytest.mark.parametrize('mutation', ['duplicate','reverse','entity','period','scenario','missing','cash'])
def test_reject_corrupt_balanced_or_scoped_outputs(mutation):
    rows = deepcopy(emitted())
    if mutation == 'duplicate': rows += deepcopy(rows[:2])
    if mutation == 'reverse':
        for row in rows[:2]: row['signed_usd'] = str(-D(row['signed_usd']))
    if mutation == 'entity': rows[0]['entity'] = 'ARU'
    if mutation == 'period': rows[0]['month'] = 9
    if mutation == 'scenario': rows[-1]['scenario'] = 'base'
    if mutation == 'missing': rows.pop()
    if mutation == 'cash': rows[0]['cash_flow'] = 'FINANCING'
...
def test_guard_opening_and_duplicate_application():
    ...
    with pytest.raises(ValueError): adjustment.post_opening(b)
    b = books('base')
