        ...
        for y in range(2026, 2032):
            ...
            for e in members:
                for j in ("US", "CA", "IL", "WV"):
                    ...
                    row = dict(
                        scenario=s, taxpayer=e, jurisdiction=j, year=y, current_tax_usd=amount
                    )
                    if j != "US":
                        states.append(row)
                    elif e != "SHI":
                        federal.append(row)
                    deferred.append(
                        dict(row, gross_dta_usd="0", valuation_allowance_usd="0", gross_dtl_usd="0")
                    )
    for m in range(1, 13):
        for n, (account, value) in enumerate(
            (("CO_SUB_TAX_CURRENT", "10"), ("CO_SUB_TAX_PAY_FED", "-10")), 1
        ):
            journal.append(
                dict(
                    scenario="base",
                    ...
                    account=account,
                    signed_usd=value,
                    source_id=f"CO-STAT-PROVISION-ARU-2026-{m}",
                    journal_id=f"J{m}",
                    line_no=n,
                )
            )
    ...
    args[0].extend(
        [
            dict(
                ...
                account=a,
                signed_usd=v,
                source_id="CO-STAT-BAD",
                journal_id="BAD",
                line_no=n,
            )
            for n, (a, v) in enumerate([("1000", "-1"), ("CO_SUB_TAX_PREPAID_FED", "1")])
        ]
    )
...
def test_native_cross_entity_deferred_bridge_is_retained():
    ...
    for entity, value in [("ARU", D(-450000)), ("BST", D(450000))]:
        for line, (account, amount, source) in enumerate(
            [
                ("1800", value, "LEGAL-DEFERRED-MOVE"),
                ("1800", -value, "CO-STAT-DEFERRED-REPLACE"),
                ("CO_SUB_TAX_DEFERRED", value, "CO-STAT-DEFERRED-REPLACE"),
            ]
        ):
            args[0].append(
                dict(
