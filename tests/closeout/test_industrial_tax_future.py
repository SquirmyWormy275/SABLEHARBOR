    ...
    return {
        "journal_rows": [
            dict(
                ...
                month=m,
                journal_id=f"{y}-{m}",
                source_id="MINE_SALES",
                signed_usd="-3647500.01",
            )
...
def posted(provider):
    return [
        dict(
            ...
            month=r["month"],
            account=a,
            signed_usd=str(D(r["tax_expense_usd"]) * sign),
        )
...
def test_native_reject(mutation):
    ...
    if mutation == "duplicate":
        ...
        source["journal_rows"][0][
            {"entity": "entity", "period": "year", "scenario": "scenario"}[mutation]
