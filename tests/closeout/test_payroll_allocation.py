...
def test_ps_employer_reclassification_preserves_group_cost_cash_and_reciprocity():
    provider = CloseoutAdjustment()
    provider.payroll_legal_correction = True
    b = Books(
        "base",
        {
            "segment_mapping": {},
            "owners": {},
            "legal_entities": ["PS", "RWH"],
            "knowledge_cutoff": "2026-09-15",
            "created_on": "2026-09-15",
        },
        provider.account_types
        | {
            "5100": "expense",
            "1150": "asset",
            "2150": "liability",
    ...
    assert sum(D(r["signed_usd"]) for r in b.rows) == 0 and all(
        r["account"] != "1000" for r in b.rows
    )
