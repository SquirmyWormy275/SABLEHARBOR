...
def holders():
    return [
        dict(
            holder_id=i,
            participation_share=s,
            participation_basis_source="TEST-ONLY-BASIS",
            basis_state="ESTABLISHED",
            rights_before={"test_right": "unchanged"},
        )
...
def test_rounding_preserves_total_and_rights_without_issuance():
    ...
    assert all(
        r["rights_after"] == r["rights_before"] and r["interests_issued"] == "0" for r in rows
    )
...
def test_reject_incomplete_or_unestablished_basis(fault):
    ...
    if fault == "duplicate":
        h.append(h[0])
    if fault == "unestablished":
        h[0]["basis_state"] = "ROUNDED_APPROXIMATION"
    if fault == "mixed":
        h[0]["participation_basis_source"] = "OTHER"
    with pytest.raises(ValueError):
        proportional_request("100.00", h)
