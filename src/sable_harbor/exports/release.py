...
def _export_csv(session: Session, table: str, destination: Path) -> int:
...
def package_public_demo(
    ...
    *,
    ...
    row_counts = {
        table: _export_csv(session, table, csv_directory / f"{table}.csv")
