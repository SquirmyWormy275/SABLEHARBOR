...
- C05: 30 activity/duty boundaries plus the eight current unit/corporate joins:
  92 contracts, 88 customers, 104 service environments, five research projects,
  702 employees and 1,404 payroll events. Current privacy/benefit data-flow facts
  replace the original missing threshold/PHI screens in their stated scope.
- All 14 existing fictional permits have authored August condition instruments:
  204 occurrences (196 performed, three failed/corrected, two missing, two open
  exceptions, one blocked), with two future duties and one zero-occurrence register
  separate. Four selected workplace examinations are included, not counted twice.
- All 14 historical rail events have authored case-specific reporting dispositions:
  nine reportable events, 11 selected incident-form records, eight timely event
  submissions and one late event; five no-trigger cases. Complete monthly railway
  reporting is not claimed. The original all-missing filing screen is superseded
  for these selected synthetic incident records.
- The August shipment hold is preserved; the selected passed 200 lb lot advances
  through a separately dated September qualification and title-retained external
  converter receipt. The former carrier/packaging gap is resolved for this lot only.
- All 92 August contract transaction-tax screens are supplied: 58 Core and 34
  industrial/material. Three rejected utility resale certificates stay rejected;
  $180,947.89 seller expense/payable is measured without new AR/cash. Finance owns
  posting, annual native-period recomputation and statutory income-tax integration.
- Fifteen formerly empty native-control risk mappings now have dated existing-risk
  relationships. This does not accept 1,660 local applicability rows: preparation
  remains 166 controls/1,660 rows; operations 70/86 controls with 24 computed adapters;
  the separate forecast exercise remains 7,560 occurrences over 42 combinations.
- C06/C09: company import contract, tenant/metadata authorization regression,
  reference disclosure tests and actual isolated filesystem restore are supplied.
  The parent receipt additionally records actual pinned CompanyStore SQLite
...
| ID | Nature | Exact residual and affected claim | Next owner/action |
|---|---|---|---|
...
| C05-R03 | Prospective adequacy and instrument-term scope | Existing bond identity, $25M face and continuation are supplied in august_admin_completion.json. Face equals the current estimate numerically; changed-condition/agency-specific adequacy remains open. A $10,000 August fee/settlement component is authored within retained RWH G&A, with $20,208 remaining; no independent bank confirmation. Underlying indemnity terms are not established. | Environmental/corporate owner completes September30 cost and applicable assurance review; no fee-zero, absent-indemnity or available-cash inference. |
...
| C06-R03 | Final edition reproduction | Existing portal receipt tests six originals against pinned module commit548cb5d, not the final expanded edition. | Integration owner rebuilds accepted edition, independently imports it and records exact counts/hashes plus current restore/revocation receipt. |
...
Remaining future responses, declared scope and runtime capability limits
affect only the corresponding claims above.

## Evidence paths and rechecks

Use `synthetic_permit_instruments_august.json`, `rail_reporting_successor.json`,
`current_activity_successor.json`, `industrial_transaction_tax.json`,
`risk_mapping_supplement.json` and their existing validators/tests. C06 parent
receipts are `docs/internal/company-closeout/REFERENCE_RESTORE_RECEIPT.json` and
`PORTAL_REHEARSAL.json`; parent generators are `tools/company_closeout/edition.py`
