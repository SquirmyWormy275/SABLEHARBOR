...
def build(output):
    ...
    opening={(r['scenario'],r['entity']):D(r['ending_cash_usd']) for r in statements
             if (int(r['year']),int(r['month']))==(2026,8)}
    closing={(r['scenario'],r['entity'],int(r['month'])):D(r['ending_cash_usd']) for r in statements
             if r['year']=='2026' and int(r['month']) in {9,10,11}}
    selected=[r for r in journal if r['account']=='1000' and r['year']=='2026' and int(r['month']) in {9,10,11}]
    ...
    if len(keys)!=len(set(keys)): raise ValueError('Duplicate cash leg in treasury population')
    daily=defaultdict(lambda:defaultdict(D))
    lineage=[]
    for r in selected:
        month=int(r['month']); count=calendar.monthrange(2026,month)[1]
        for i,value in enumerate(spread(r['signed_usd'],count)):
            ...
            for timing in ['UNIFORM_CALENDAR_ALLOCATION','MEMBER_CASH_UNAVAILABLE']:
                missing=(timing=='MEMBER_CASH_UNAVAILABLE' and r['entity']=='SHI' and r['source_type']=='MEMBER_EQUITY')
                ...
                if r['entity']=='SHI' and r['source_type']=='MEMBER_EQUITY': fields['member_cash_usd'] += value
        lineage.append({k:r[k] for k in ['scenario','entity','year','month','journal_id','line_no','source_id','source_type','cash_flow','signed_usd','available_at','fact_state']})
    ...
    for r in selected:
        consolidated[r['scenario'],int(r['month']),r['source_id'],r['cash_flow']] += D(r['signed_usd'])
    member_ids={r['source_id'] for r in selected if r['entity']=='SHI' and r['source_type']=='MEMBER_EQUITY'}
    for (scenario,month,source_id,flow),amount in consolidated.items():
      for i,value in enumerate(spread(amount,calendar.monthrange(2026,month)[1])):
       for timing in ['UNIFORM_CALENDAR_ALLOCATION','MEMBER_CASH_UNAVAILABLE']:
        ...
        if source_id in member_ids: fields['member_cash_usd']+=value
    result=[]; month_checks=[]
    for timing in ['UNIFORM_CALENDAR_ALLOCATION','MEMBER_CASH_UNAVAILABLE']:
      for (scenario,entity),initial in sorted(opening.items()):
        if entity=='ELIM':continue
        cash=initial; cumulative_missing=D(0)
        for week in range(13):
            start=START+timedelta(days=week*7);end=start+timedelta(days=6)
            values=defaultdict(D);week_open=cash;minimum=cash
            for offset in range(7):
                day=start+timedelta(days=offset);flow=daily[timing,scenario,entity,day]
                for key,value in flow.items():values[key]+=value
                ...
                if day.day==calendar.monthrange(day.year,day.month)[1]:
                    ...
                    if cash!=expected:raise ValueError(f'Month closing does not reconcile {timing}/{scenario}/{entity}/{day}: {cash} != {expected}')
                    month_checks.append(dict(timing_case=timing,scenario=scenario,entity=entity,month=day.month,
                      monthly_source_closing_usd=str(closing[scenario,entity,day.month]),
                      unavailable_member_cash_cumulative_usd=str(cumulative_missing),
                      timing_closing_usd=str(cash),difference_usd='0.0000'))
            result.append(dict(timing_case=timing,scenario=scenario,entity=entity,week=week+1,
              week_start=str(start),week_end=str(end),timezone='America/Los_Angeles',
              opening_cash_usd=str(week_open),receipts_usd=str(values['receipts_usd']),
              payments_usd=str(-values['payments_signed_usd']),closing_cash_usd=str(cash),
              arithmetic_difference_usd=str(week_open+values['receipts_usd']+values['payments_signed_usd']-cash),
              ...
              liquidity_state='INFEASIBLE_WITHOUT_RESPONSE' if minimum<0 else 'NO_NEGATIVE_UNDER_ASSUMED_TIMING',
              **{k:str(values[k]) for k in ['operating_net_usd','investing_net_usd','financing_net_usd']},
              evidence_state='MONTHLY_MODEL_TIMING_SENSITIVITY_NOT_BANK_CONFIRMATION',
              known_on='2026-09-15',period_role='CONDITIONAL_FORECAST',
    ...
    return {'weeks':13,'start':str(START),'end':str(END),'rows':len(result),
      'selected_cash_legs':len(lineage),'monthly_checks':len(month_checks),
      'negative_week_rows':sum(r['liquidity_state']=='INFEASIBLE_WITHOUT_RESPONSE' for r in result),
