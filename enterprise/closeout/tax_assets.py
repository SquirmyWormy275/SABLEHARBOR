...
def federal_depreciation(cost, placed_year, year):
    ...
    if year < placed_year:
        return D(0)
    if placed_year >= 2025:
        return cost if year == placed_year else D(0)
    if placed_year != 2023:
        raise ValueError("Unreviewed historical property year")
...
def build(result, operating=None):
    cohorts = []
    events = {r["event_id"]: r for r in operating.tables["events"]} if operating else {}
    asset_source = {r["asset_id"]: r for r in operating.inputs["assets"]} if operating else {}
    groups = defaultdict(list)
    for r in result["journal_rows"]:
        if r["entity"] == "SHI":
            groups[r["scenario"], r["journal_id"]].append(r)
    legacy_cases = {
        r["scenario"]
        for r in result["journal_rows"]
        if r["entity"] == "SHI"
        and int(r["month"]) == 0
        and r["account"] == "LEG_1500"
        and D(r["signed_usd"]) == 9000000
    }
    for case in sorted(legacy_cases):
        cohorts.append(
            dict(
                scenario=case,
                asset_id="LEGACY-OWNED-EQUIPMENT-9M",
                source_id="LEGACY-OPEN-2026",
                cost=D(9000000),
                placed_year=2023,
                placed_month=1,
                life_months=84,
    ...
    for (case, journal), rows in groups.items():
        for r in rows:
            if int(r["month"]) == 0 or D(r["signed_usd"]) <= 0:
                continue
            account = r["account"]
            eligible = account in {"RT_IT", "LEG_1500"} or (
                account == "BIZ_PPE"
                and any(x["account"] == "1000" and D(x["signed_usd"]) < 0 for x in rows)
            )
            if not eligible:
                continue
            event = events.get(r["source_id"], {})
            aid = event.get("source_id", r["source_id"])
            ...
            cohorts.append(
                dict(
                    ...
                    cost=D(r["signed_usd"]),
                    placed_year=int(r["year"]),
                    placed_month=int(r["month"]),
                    life_months=int(life),
    ...
    for case in ["base", "downside", "expansion"]:
        ...
        for c in selected:
            prior_fed += sum(
                (federal_depreciation(c["cost"], c["placed_year"], y) for y in range(2023, 2026)),
                D(0),
            )
            prior_ca += sum(
                (
                    ca_depreciation(
                        c["cost"], c["placed_year"], c["placed_month"], c["life_months"], y
                    )
        ...
        opening[case] = {
            "federal_depreciation": prior_fed.quantize(Q),
            "ca_depreciation": prior_ca.quantize(Q),
            "book_net": book.get((case, 2026, 0), D(0)),
            "federal_basis": (
                sum((c["cost"] for c in selected if c["placed_year"] < 2026), D(0)) - prior_fed
            ).quantize(Q),
            "ca_basis": (
                sum((c["cost"] for c in selected if c["placed_year"] < 2026), D(0)) - prior_ca
            ).quantize(Q),
        }
        for year in range(2026, 2032):
            fd = cd = fb = cb = cost = D(0)
            for c in selected:
                if c["placed_year"] > year:
                    continue
                f = federal_depreciation(c["cost"], c["placed_year"], year).quantize(Q)
                ca = ca_depreciation(
                    c["cost"], c["placed_year"], c["placed_month"], c["life_months"], year
                ).quantize(Q)
                fbase = max(
                    c["cost"]
                    - sum(
                        (
                            federal_depreciation(c["cost"], c["placed_year"], y)
                            for y in range(c["placed_year"], year + 1)
                        ),
                        D(0),
                    ),
                    D(0),
                ...
                annual.append(
                    ...
                    | dict(
                        year=year,
                        federal_depreciation_usd=str(f),
                        california_depreciation_usd=str(ca),
                        federal_closing_basis_usd=str(fbase),
                        california_closing_basis_usd=str(cbase),
                    )
                )
            totals[case, year] = dict(
                federal_depreciation=fd,
                ca_depreciation=cd,
                federal_basis=fb,
                ca_basis=cb,
                book_net=book[case, year, 12],
                original_cost=cost,
            )
