...
import shutil
from enterprise.operations import exports, credit, research, management, controls
...
def build(output, operating, successor, op, fin, bridge, identity):
    industrial=transactions.build(output/'industrial/transactions',
        operating_rows=op['operating_rows'],forecast=fin)
    credit.allocate_treasury(operating,successor,
        additional_deferrable_source_types=('RUNTIME_CONDITIONAL_FORECAST_REQUEST',))
    research.build_industrial_detail(operating,op['operating_rows'],industrial['tables'],fin['journal_rows'])
    management.build(operating,successor,model_factory=OperatingModel)
    controls.build_controls(operating,successor,extras={'industrial_operations':op['operating_rows'],'replacement_bridge':bridge})
    ...
    exports.write_json(target/'coverage.json',{'table_counts':{k:len(v) for k,v in tables.items()},'unit_counts':counts,
      ...
      'limitations':['Source operating populations retain their own effective/available periods and are not all current headcount.',
