...
TAX_TYPES={'CO_TAX_CURRENT':'expense','CO_TAX_PAY_FED':'liability','CO_TAX_PAY_CA':'liability',
 'CO_TAX_STATE_RESERVE':'liability','CO_TAX_DTA':'asset','CO_TAX_VA':'asset','CO_TAX_DEFERRED':'expense','CO_TAX_VA_EXP':'expense'}


class ParentTax:
    def __init__(self,result,legacy):
        ...
        self.opening_nol=-sum(historic.values())
        self.opening_dta=(self.opening_nol*(D('.21')+D('.0884')*D('.79'))).quantize(Q)
        ...
        for scenario in ['base','downside','expansion']:
            fnol=snol=self.opening_nol;interest_cf=D(0);temporary=D(0);previous_dta=self.opening_dta
            for year in range(2026,2032):
                ...
                ca_used=min(snol,max(common,D(0))) if not (year<=2026 and common>=1000000) else D(0)
                snol+=max(-common,D(0))-ca_used
                ca_exposure=max(D(800),(max(common,D(0))-ca_used)*D('.0884')).quantize(Q)
                ca=D(800)
                state_reserve=ca_exposure-ca
                # ATI adds back interest; DDA already removed in this reserved deduction calculation.
                ati=max(common-ca+interest,D(0));limit=ati*D('.30')
                interest_used=min(interest+interest_cf,limit)
                interest_cf+=interest-interest_used
                fed_before=common-ca+interest-interest_used
                ...
                record=dict(scenario=scenario,entity='SHI',year=year,book_pretax_usd=str(pretax),
                  book_depreciation_unrecognized_deduction_usd=str(dda),allowance_addback_usd=str(allowance),
                  ...
                  california_unallocated_reserve_usd=str(state_reserve),current_expense_usd=str(fed+ca+state_reserve),gross_dta_usd=str(dta),valuation_allowance_usd=str(dta),
                  gross_deferred_expense_usd=str(previous_dta-dta),valuation_allowance_expense_usd=str(dta-previous_dta),
                  net_deferred_expense_usd='0.0000',net_dta_usd='0.0000',
                  status='ADOPTED_DIRECTION_SYNTHETIC_PROVISION_WITH_DISCLOSED_TAX_BASIS_AND_STATE_RESERVATIONS')
                self.rows.append(record)
                fed_parts=self.parts(fed);ca_parts=self.parts(ca);dta_parts=self.parts(dta-previous_dta);reserve_parts=self.parts(state_reserve)
                for month in range(1,13):
                    self.monthly[scenario,year,month]=(fed_parts[month-1],ca_parts[month-1],dta_parts[month-1],
                        ...
                        (ca*D('.3')).quantize(Q) if month==4 else (ca*D('.4')).quantize(Q) if month==6 else ca-(ca*D('.3')).quantize(Q)-(ca*D('.4')).quantize(Q) if month==12 else D(0),reserve_parts[month-1])
                previous_dta=dta
    ...
    def post_opening(self,books):
        ...
        books.post('SHI',2026,0,[('CO_TAX_DTA',self.opening_dta),('CO_TAX_VA',-self.opening_dta)],'CO-TAX-OPENING-NOL',
          'Supported 2023-2025 corporate loss population; full allowance, no net asset or pre-2023 deduction inferred',kind='COMPANY_TAX_ADJUSTMENT')
    ...
    def post_month(self,books,year,month):
        ...
        fed,ca,dta,fp,cp,state_reserve=self.monthly[books.scenario,year,month]
        ...
        if dta:
            books.post('SHI',year,month,[('CO_TAX_DTA',dta),('CO_TAX_DEFERRED',-dta),('CO_TAX_VA',-dta),('CO_TAX_VA_EXP',dta)],
              f'CO-TAX-DEFERRED-{year}-{month}','Gross loss/temporary-difference DTA and full valuation allowance',kind='COMPANY_TAX_ADJUSTMENT')
        if fp+cp:
