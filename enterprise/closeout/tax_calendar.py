...
def business_day(d, dc=False):
    holidays = {
        observed(date(d.year, 1, 1)),
        observed(date(d.year, 7, 4)),
        observed(date(d.year, 12, 25)),
    }
    if dc:
        holidays.add(observed(date(d.year, 4, 16)))
...
def return_due(year, jurisdiction):
    if jurisdiction in {"US", "CA"}:
        ...
        if year == 2019:
            return date(2020, 7, 15)
        return business_day(date(year + 1, 4, 15), dc=True)
    if jurisdiction == "PA":
        return business_day(date(year + 1, 5, 15))
...
def build():
    ...
    def add(entity, jurisdiction, year, form, due, classification, source, history=False, scope=""):
        ...
        if history and not future:
            ...
            prep = date(year + 1, 3, 10).isoformat()
            sub = business_day(date(year + 1, 4, 1)).isoformat()
            ack = (date.fromisoformat(sub) + timedelta(days=1)).isoformat()
            state = "AUTHORED_MODELED_SUBMISSION_AND_ACKNOWLEDGEMENT"
        rows.append(
            dict(
                filing_id=f"CO-FILING-{entity}-{jurisdiction}-{year}-{form}",
                entity=entity,
                jurisdiction=jurisdiction,
                tax_year=year,
                form=form,
                classification=classification,
                ordinary_due_on=due.isoformat(),
                prepared_on=prep,
                modeled_submission_on=sub,
                modeled_acknowledgement_on=ack,
                state=state,
                known_on="2026-09-15",
                authority_url=source,
                authority_accessed_on="2026-09-15",
                future_law_assumption="HOLDS_REVIEWED_LAW_CONSTANT"
                if year >= 2026
    ...
    for year in range(2016, 2032):
        for jurisdiction, form in [("US", "1120"), ("CA", "100")]:
            source = (
                ...
                if jurisdiction == "US" and year <= 2017
                else "https://www.ftb.ca.gov/forms/2016/16_100bk.pdf"
                if jurisdiction == "CA" and year == 2016
                else AUTH[jurisdiction]
            )
            add(
                "SHI",
                jurisdiction,
                year,
                form,
                return_due(year, jurisdiction),
                "LLC_TAXED_C_CORPORATION",
                source,
                history=year <= 2025 if jurisdiction == "US" else year <= 2023,
                scope="Initial2016election; laterCAcombinedperimeter must reconcile before2024+submission",
            )
        if year >= 2020:
            add(
                "SHI",
        ...
        add(
            ...
            "LLC300",
            date(year + 1, 6, 1),
            "LEGAL_LLC_ANNUAL_TAX_NO_CORPORATE_REPORT",
            AUTH["DE"],
    ...
    add(
        ...
        "DISREGARDED_IN_PS",
        AUTH["US"],
        scope="Legaloperator persists; no separatecorporateincomereturn; employment/exciseobligationsdistinct",
    )
    ...
    add(
        ...
        "8023",
        date(2026, 10, 15),
        "INTENDED338H10_CONDITIONAL_ELIGIBILITY",
        "https://www.irs.gov/instructions/i8023",
        scope="Jan7acquisition;15thdayninthmonthafteracquisitionmonth; sellersandbuyerjointsignature/submissionrequired",
    )
