...
def run(out):
    ...
    for row in predecessor:
        if (row['scenario'],row['entity'],row['year'],row['account'],row['source_type']) == ('base','SHI','2027','1000','MEMBER_EQUITY'):
    ...
    result['scope'] = 'Targeted lane evidence; not a full tax provision, accepted edition or bank confirmation'
    return result
