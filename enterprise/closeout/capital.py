...
def proportional_request(total, holders):
    ...
    if any(not h['participation_basis_source'] or h['basis_state'] != 'ESTABLISHED' for h in holders):
    ...
    for k in order[:remainder]: cents[k] += D('.01')
    return [dict(h,requested_usd=str(cents[h['holder_id']]),
                 commitment_usd=None,received_usd='0.00',interests_issued='0',
                 event_state='PREPARED_VOLUNTARY_REQUEST',
                 rights_after=h['rights_before']) for h in sorted(holders,key=lambda h:h['holder_id'])]
