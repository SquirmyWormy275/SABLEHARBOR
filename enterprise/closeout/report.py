...
def report(out):
    ...
    for row in rows:
        ...
        if (
            ...
            and row["source_type"] == "MEMBER_EQUITY"
        ):
        ...
        result.append(
            {
                "scenario": key[0],
                "year": key[1],
                "member_cash_usd": str(funding[key]),
                "member_cash_cumulative_usd": str(cumulative[key[0]]),
                ...
                "cash_after_modeled_operations_and_all_net_investment_usd": str(
                    operating + investing
                ),
                "ending_cash_usd": row["ending_cash_usd"],
                "dependence_ratio_to_positive_operating_cash": str(funding[key] / operating)
                if operating > 0
                else "N/A",
                "change_in_member_cash_usd": str(
                    funding[key] - funding.get((key[0], str(int(key[1]) - 1)), D(0))
                )
                if int(key[1]) > 2026
                else "N/A",
