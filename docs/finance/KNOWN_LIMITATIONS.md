...
- Summary journals remain available as a compact calibration profile. The standard profile now
  ...
  Explicit repayments now accompany each forecast debt draw. Aging distinguishes formal invoice/
  vendor-bill documents from source-event operational receivables and accruals, and reconciles both
  to the GL with zero unallocated balance. Debt schedules distinguish causal forecast facilities
