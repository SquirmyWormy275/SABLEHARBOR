...
class Exporter:
    ...
    def reading_pages(self) -> dict[str, str]:
        result = {}
        groups = {}
        topic_path = self.root / "tools/wiki/topics.json"
        topics = json.loads(topic_path.read_text()) if topic_path.exists() else {}
        ...
        for group, sources in sorted(groups.items()):
            ...
            rows = [f"# {topic['title']}", "", "[All topics](Reading) · [Wiki home](Home)", "", topic["introduction"], "", "## Read the records", ""]
    ...
    def polish_titles(self, content):
        config = self.root / "tools/wiki/titles.json"
        if not config.exists():
            return content, {}
        titles = json.loads(config.read_text())
        legacy_path = self.root / "tools/wiki/legacy-sections.json"
        legacy = json.loads(legacy_path.read_text())["routes"] if legacy_path.exists() else {}
        unknown = titles.keys() - {Path(name).stem for name in content}
        if unknown:
            raise ValueError(f"Title inventory refers to absent pages: {sorted(unknown)}")
        names = {Path(name).stem: titles.get(Path(name).stem, Path(name).stem) for name in content}
        if len({name.casefold() for name in names.values()}) != len(names):
            raise ValueError("Polished Wiki titles collide")
        if any(not re.fullmatch(r"[A-Za-z0-9_.-]+", name) for name in names.values()):
            raise ValueError("Unsafe polished Wiki page title")
        # A new canonical name must not take over another page's historical URL.
        for old, new in names.items():
            if old != new and new in names:
                raise ValueError(f"Canonical title shadows a historical page: {new}")

        def destination(value):
            parsed = urlsplit(html.unescape(value))
            prefix = f"https://github.com/{REPOSITORY}/wiki/"
            if value.startswith(prefix):
                old = unquote(parsed.path.rsplit("/", 1)[-1])
                if old in names:
                    return urlunsplit((*urlsplit(prefix + quote(names[old]))[:3], parsed.query, parsed.fragment))
            elif not parsed.scheme and not parsed.netloc and unquote(parsed.path) in names:
                return urlunsplit(("", "", quote(names[unquote(parsed.path)]), parsed.query, parsed.fragment))
            return value

        def prose(value):
            def link(match):
                bracketed = match[2].startswith("<")
                value = destination(match[2][1:-1] if bracketed else match[2])
                return match[1] + (f"<{value}>" if bracketed else value)
            value = INLINE.sub(link, value)
            value = REFERENCE.sub(link, value)
            return ATTRIBUTE.sub(lambda m: f"{m[1]}={m[2]}{destination(m[3])}{m[2]}", value)

        result, aliases = {}, {}
        for filename, body in content.items():
            old = Path(filename).stem
            chunks, offset = [], 0
            for match in CODE.finditer(body):
                chunks.extend((prose(body[offset:match.start()]), match[0]))
                offset = match.end()
            chunks.append(prose(body[offset:]))
            result[names[old] + ".md"] = "".join(chunks)
            if old != names[old]:
                aliases[filename] = names[old] + ".md"
                target = f"https://github.com/{REPOSITORY}/wiki/{quote(names[old])}"
                label = names[old].replace("-", " ")
                rows = ["# This page has a new title", "", f"Continue to **[{label}]({target})**.", "", "This address is retained for existing bookmarks. The article and its source history remain available at the link above.", "", "<details>", "<summary>Links to the original sections</summary>", ""]
                anchors = Page(body).anchors
                previous = legacy.get(old, {})
                if any(target not in anchors for target in previous.values()):
                    raise ValueError(f"Historical section target is missing in {old}")
                for anchor in sorted(anchors | previous.keys()):
                    rows.append(f'<a id="{html.escape(anchor, quote=True)}"></a>')
                    rows.append(f"- [Continue to {anchor.replace('-', ' ')}]({target}#{quote(previous.get(anchor, anchor))})")
                rows.extend(["", "</details>", ""])
                result[filename] = "\n".join(rows)
    ...
    def publication_inputs(self):
        ...
        self.inputs.update(p for p in (self.root / "tools/wiki").glob("*.py"))
        self.inputs.update(p for p in (self.root / "tools/wiki").glob("*.json"))
        inputs = {str(p.relative_to(self.root)): hashlib.sha256(p.read_bytes()).hexdigest()
                  for p in sorted(self.inputs)}
        trees = {}
        for path in sorted(self.directories):
            relative = str(path.relative_to(self.root))
            result = subprocess.run(["git", "ls-tree", "HEAD", "--", relative], cwd=self.root,
                                    capture_output=True, text=True)
            if result.returncode == 0 and result.stdout:
                trees[relative] = hashlib.sha256(result.stdout.encode()).hexdigest()
        return inputs, trees
    ...
    def build(self, output: Path) -> dict:
        ...
        sidebar = [
            ...
            "[Home](Home)",
            "",
            *(["- [Start here](Start-Here)", "- [Open questions](Open-Questions)", "- [Reading glossary](Glossary)"] if (self.wiki / "Start-Here.md").exists() else []),
        ...
        inputs, directory_trees = self.publication_inputs()
        ...
        manifest = {
            ...
            "reading_sources": {
                str(p.relative_to(self.root)): {
                    "page": aliases.get(self.names[p] + ".md", self.names[p] + ".md"),
